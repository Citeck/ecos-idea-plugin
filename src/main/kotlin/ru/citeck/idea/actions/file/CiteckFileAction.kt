package ru.citeck.idea.actions.file
...
abstract class CiteckFileAction : AnAction() {
    ...
    override fun actionPerformed(event: AnActionEvent) {
        ...
        try {
            ...
            if (rootCause is GracefulAbortException) {
                ...
                EcosMessages.error("Action error", errorMsg, event.project)
            }
        }
    }
    ...
    fun resolveFileType(event: AnActionEvent): FileType {
        return FileTypeService.getInstance().getFileType(getPsiFile(event))
    }
}
