...
class ActionJsonSchemaProvider(private val project: Project) : JsonSchemaFileProvider {

    override fun isAvailable(file: VirtualFile): Boolean {
        val meta = ArtifactsService.getInstance().getArtifactInfo(file, project) ?: return false
        return meta.getTypeId() == "ui/action"
    }

    override fun getName(): String = "Citeck Action Schema"

    override fun getSchemaFile(): VirtualFile? {
        return JsonSchemaProviderFactory.getResourceFile(
            ActionJsonSchemaProvider::class.java,
            "/citeck/artifacts/ui/action/action-schema.json"
        )
    }
